...
public class AutoPackTexturePool extends TexturePool
{
	...
	public void addAll(List<TexturePool.MsgTexture> list) {
		// TODO: Implement this method
		List<MsgTexture> rlist=new ArrayList<MsgTexture>();
		for(MsgTexture t:list){
			if(t.width<t.height){
				t.swapXY=true;
				int tmp=t.width;
		...
		Collections.sort(rlist, new Comparator<MsgTexture>(){
				@Override
				public int compare(TexturePool.MsgTexture p1,TexturePool.MsgTexture p2) {
					// TODO: Implement this method
	...
	private AbstractTexture tryAddInLine(MsgTexture m){
		if(currentY+m.height<currentPack.layer.getWidth()){
			packCanvas.prepare();
			packCanvas.save();
			//packCanvas.getData().getShaders().setTexture3DShader(ShaderManager.getRawTextureShader());
			Quad texq=RectF.xywh(currentX,currentY,m.width,m.height).toQuad();
			if(m.swapXY)texq.swapXY();
			packCanvas.drawTexture(m.texture,texq,rawPaint);
			packCanvas.restore();
			packCanvas.unprepare();
			RectI area=RectI.xywh(currentX,currentY,m.width,m.height);
			AbstractTexture t=new TextureRegion(
											currentPack.layer.getTexture().getTexture(),
											//textureTest,
											area);
	...
	private AbstractTexture tryAddToPack(MsgTexture m){
		if(currentX+m.width<currentPack.layer.getWidth()){
			return tryAddInLine(raw,msg);
		}else{
			toNextLine();
			return tryAddToPack(m);
		}
	}
	...
	private AbstractTexture testAddRaw(MsgTexture m){
		if(m.width>=getMaxWidth()||m.height>=getMaxHeight()){
			return m.texture;
		}else{
			return tryAddToPack(m);
		}
	...
	protected AbstractTexture loadTexture(String msg) {
		// TODO: Implement this method
		AbstractTexture raw=super.loadTexture(msg);
		return testAddRaw(new MsgTexture(msg,raw));
	}
